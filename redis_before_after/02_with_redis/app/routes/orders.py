...
██ ФАЙЛ НЕ ИЗМЕНИЛСЯ — точная копия из 01_no_cache ██

Почему заказы НЕ кэшируются:
  1. POST /orders/ — операция ЗАПИСИ, кэш не применим
  2. GET /orders/  — данные меняются с КАЖДЫМ заказом (минуты)
  3. GET /orders/{id} — персональные данные конкретного пользователя,
     запрашиваются 1-2 раза и больше не нужны (нет повторных хитов)

Redis полезен только когда МНОГО разных пользователей запрашивают
ОДНИ И ТЕ ЖЕ данные. Заказы — полная противоположность.
"""
