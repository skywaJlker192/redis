"""
Роуты книг.
С Redis-кэшем на ГОРЯЧИХ эндпоинтах.

██ ИЗМЕНЁННЫЙ ФАЙЛ — сравни с task/app/routes/books.py ██

Кэш добавлен ТОЛЬКО на:
  • GET /books/top-rated    — главная страница, все видят одно и то же
  • GET /books/popular      — блок «Популярное», одинаков для всех
  • GET /books/{id}         — карточка книги, популярные книги запрашиваются часто

Кэш НЕ добавлен на:
  • GET /books/             — пагинация + фильтр = слишком много комбинаций ключей
  • GET /books/count        — лёгкий запрос, кэш не оправдан
  • GET /books/search?q=    — уникальные строки поиска → низкий hit rate
  • POST /books/            — операция записи
  • PATCH /books/{id}       — операция записи (но добавлена ИНВАЛИДАЦИЯ кэша!)
"""

from __future__ import annotations

from fastapi import APIRouter, Depends, HTTPException, Query
from sqlalchemy import select, func
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.orm import joinedload

from app.db import get_session
from app.models import Book
from app.schemas import BookCreate, BookUpdate

# ── НОВОЕ: импорт кэш-функций ────────────────────────────
from app.cache_service import (
    get_cached_top_rated, set_cached_top_rated,
    get_cached_popular, set_cached_popular,
    get_cached_book, set_cached_book,
    invalidate_book,
)
# ──────────────────────────────────────────────────────────

router = APIRouter(prefix="/books", tags=["books"])


# ── READ endpoints ────────────────────────────────────────

@router.get("/")
async def list_books(
    author_id: int | None = Query(None),
    limit: int = Query(20, ge=1, le=100),
    offset: int = Query(0, ge=0),
    session: AsyncSession = Depends(get_session),
):
    """
    Список книг с пагинацией и фильтром по автору.

    ██ БЕЗ КЭША ██
    Почему: комбинации author_id × limit × offset создают
    слишком много уникальных ключей. Кэш будет неэффективным
    (низкий hit rate, много памяти).
    """
    stmt = (
        select(Book)
        .options(joinedload(Book.author))
        .where(Book.is_available.is_(True))
        .order_by(Book.id)
        .offset(offset)
        .limit(limit)
    )
    if author_id:
        stmt = stmt.where(Book.author_id == author_id)

    result = await session.execute(stmt)
    books = result.unique().scalars().all()
    return [b.to_dict() for b in books]


@router.get("/top-rated")
async def top_rated_books(
    limit: int = Query(10, ge=1, le=50),
    session: AsyncSession = Depends(get_session),
):
    """
    Топ книг по рейтингу.

    ██ ДОБАВЛЕН КЭШ (Cache-Aside): ██
    Показывается на ГЛАВНОЙ СТРАНИЦЕ — горячий эндпоинт.
    Одинаковый ответ для всех → идеальный кандидат.
    Redis TTL = 10 мин.
    """
    # ── НОВОЕ: проверяем кэш ──────────────────────────────
    cached = await get_cached_top_rated(limit)
    if cached is not None:
        return cached
    # ──────────────────────────────────────────────────────

    result = await session.execute(
        select(Book)
        .options(joinedload(Book.author))
        .where(Book.is_available.is_(True))
        .order_by(Book.rating.desc())
        .limit(limit)
    )
    books = result.unique().scalars().all()
    data = [b.to_dict() for b in books]

    # ── НОВОЕ: сохраняем в кэш ───────────────────────────
    await set_cached_top_rated(limit, data)
    # ──────────────────────────────────────────────────────

    return data


@router.get("/popular")
async def popular_books(
    limit: int = Query(10, ge=1, le=50),
    session: AsyncSession = Depends(get_session),
):
    """
    Самые просматриваемые книги.

    ██ ДОБАВЛЕН КЭШ (Cache-Aside): ██
    Блок «Популярное» на главной — одинаков для всех.
    Redis TTL = 5 мин (views_count меняется чаще).
    """
    # ── НОВОЕ: проверяем кэш ──────────────────────────────
    cached = await get_cached_popular(limit)
    if cached is not None:
        return cached
    # ──────────────────────────────────────────────────────

    result = await session.execute(
        select(Book)
        .options(joinedload(Book.author))
        .where(Book.is_available.is_(True))
        .order_by(Book.views_count.desc())
        .limit(limit)
    )
    books = result.unique().scalars().all()
    data = [b.to_dict() for b in books]

    # ── НОВОЕ: сохраняем в кэш ───────────────────────────
    await set_cached_popular(limit, data)
    # ──────────────────────────────────────────────────────

    return data


@router.get("/count")
async def books_count(session: AsyncSession = Depends(get_session)):
    """
    Общее количество доступных книг.

    ██ БЕЗ КЭША ██
    Почему: запрос COUNT(*) очень быстрый в PostgreSQL,
    кэширование не даст заметного выигрыша.
    """
    result = await session.execute(
        select(func.count()).select_from(Book).where(Book.is_available.is_(True))
    )
    return {"count": result.scalar()}


@router.get("/search")
async def search_books(
    q: str = Query(..., min_length=2),
    limit: int = Query(20, ge=1, le=100),
    session: AsyncSession = Depends(get_session),
):
    """
    Поиск книг по названию.

    ██ БЕЗ КЭША ██
    Почему: каждый пользователь ищет разное → тысячи уникальных ключей.
    Hit rate будет крайне низким, а память Redis забьётся.
    """
    result = await session.execute(
        select(Book)
        .options(joinedload(Book.author))
        .where(Book.title.ilike(f"%{q}%"))
        .order_by(Book.rating.desc())
        .limit(limit)
    )
    books = result.unique().scalars().all()
    return [b.to_dict() for b in books]


@router.get("/{book_id}")
async def get_book(
    book_id: int,
    session: AsyncSession = Depends(get_session),
):
    """
    Карточка книги по ID.

    ██ ДОБАВЛЕН КЭШ (Cache-Aside): ██
    Популярные книги запрашиваются тысячами пользователей.
    Redis TTL = 10 мин.
    """
    # ── НОВОЕ: проверяем кэш ──────────────────────────────
    cached = await get_cached_book(book_id)
    if cached is not None:
        return cached
    # ──────────────────────────────────────────────────────

    result = await session.execute(
        select(Book)
        .options(joinedload(Book.author))
        .where(Book.id == book_id)
    )
    book = result.unique().scalar_one_or_none()
    if not book:
        raise HTTPException(404, "Book not found")

    # Увеличиваем счётчик просмотров
    book.views_count += 1
    await session.commit()

    data = book.to_dict()

    # ── НОВОЕ: сохраняем в кэш ───────────────────────────
    await set_cached_book(book_id, data)
    # ──────────────────────────────────────────────────────

    return data


# ── WRITE endpoints ───────────────────────────────────────

@router.post("/", status_code=201)
async def create_book(
    data: BookCreate,
    session: AsyncSession = Depends(get_session),
):
    """
    Добавить книгу.

    ██ БЕЗ КЭША ██ — операция записи.
    """
    book = Book(
        title=data.title,
        description=data.description,
        isbn=data.isbn,
        author_id=data.author_id,
        year=data.year,
        rating=data.rating,
    )
    session.add(book)
    await session.commit()
    await session.refresh(book, attribute_names=["author"])
    return book.to_dict()


@router.patch("/{book_id}")
async def update_book(
    book_id: int,
    data: BookUpdate,
    session: AsyncSession = Depends(get_session),
):
    """
    Обновить книгу.

    ██ БЕЗ КЭША, НО С ИНВАЛИДАЦИЕЙ ██
    После обновления в БД нужно удалить старый кэш,
    иначе пользователи увидят устаревшие данные.
    """
    result = await session.execute(
        select(Book).where(Book.id == book_id)
    )
    book = result.scalar_one_or_none()
    if not book:
        raise HTTPException(404, "Book not found")

    update_data = data.model_dump(exclude_unset=True)
    for field, value in update_data.items():
        setattr(book, field, value)

    await session.commit()
    await session.refresh(book, attribute_names=["author"])

    # ── НОВОЕ: инвалидация кэша после записи ─────────────
    await invalidate_book(book_id)
    # ──────────────────────────────────────────────────────

    return book.to_dict()
